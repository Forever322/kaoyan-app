/**
 * 匹配引擎
 * 根据用户输入（分数、学位、门类、专业、分区）匹配院校并排序
 */

function matchUniversities(score, degree, category, zone, major) {
  // 1. 查国家线
...
function evaluateMatch(userScore, admissionScores) {
  if (!admissionScores || admissionScores.length === 0) {
    return {
      verdict: 'nodata', label: '无数据', cssClass: '',
      avgScore: null, maxScore: null, minScore: null
    };
  }
  ...
  if (userScore >= maxScore + 10) {
    return { verdict: 'safe', label: '稳过', cssClass: 'verdict-safe', avgScore, maxScore, minScore };
  } else if (userScore >= maxScore) {
    return { verdict: 'likely', label: '大概率', cssClass: 'verdict-likely', avgScore, maxScore, minScore };
  } else if (userScore >= minScore) {
    return { verdict: 'reach', label: '冲刺', cssClass: 'verdict-reach', avgScore, maxScore, minScore };
  } else {
    return { verdict: 'unmatched', label: '差距较大', cssClass: 'verdict-fail', avgScore, maxScore, minScore };
  }
}
